 ...
 */
 ...
 setlocale(LC_ALL, 'C.UTF-8', 'C');
    ...
    // CSS buttons when selected in configuration
    ...
    if ($layout_files_text != ''){
        ...
        for ($i = 0; $i < count($items); $i++) {
            ...
            if ($i > 0){
                ...
                // Add pagination
                $html_file = JDHelper::insertPagination($params->get('option_navigate_top'), $this->params->get('show_pagination'), $this->params->get('show_pagination_results'), $this->pagination, $html_file);
            ...
            if ($items[$i]->params->get('access-download') == true){ 
                ...
                if ($has_no_file || !$items[$i]->state){
                    ...
                    $html_file = str_replace('{url_download}', '', $html_file); 
                     ...
                     $html_file = str_replace('{url_download}',$download_link_text.$pic_downloads.JText::_('COM_JDOWNLOADS_LINKTEXT_DOWNLOAD_URL').'</a>', $html_file);  
                }    
            ...
            if ($params->get('view_detailsite')){
                ...
                if ($user_can_see_download_url && !$has_no_file){
                   ...
                   if ($url_task == 'download.send'){ 
                      $download_link_text = '<a '.$blank_window.' href="'.$download_link.'" title="'.JText::_('COM_JDOWNLOADS_LINKTEXT_DOWNLOAD_URL').'" class="jd_download_url">'.$items[$i]->title.'</a>';
                   } else {
            ...
            if ($params->get('activate_general_plugin_support')) {  
                $html_file = JHtml::_('content.prepare', $html_file, '', 'com_jdownloads.mydownloads');
