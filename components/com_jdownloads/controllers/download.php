...
class jdownloadsControllerDownload extends JControllerForm
{
    ...
    public function send()
    {
        ...
        // Make sure that we have only a valid comma seperated integer value list
           ...
           if (!$mirror){
               if ($files[0]->url_download){
                   ...
                   if ($files[0]->catid > 1){
                       ...
                       $filename = $params->get('files_uploaddir').'/'.$params->get('uncategorised_files_folder_name').'/'.$files[0]->url_download;
                   }    
