...
class JDHelper
{
    ...
    */
    public static function buildSubCategorySelectBox($catlistid, $subcat_listid, $sub_categories, $cat_link_itemids, $root_itemid, $show_empty_categories, $orderby_pri)
    {
        ...
        $preload[] = JHtml::_('select.option', 0, JText::_('COM_JDOWNLOADS_FE_SELECT_SUB_CATEGORY'));
        $selected[] = JHtml::_('select.option', $subcat_listid );
    ...
    */
    public static function buildFieldTitles($html, $file)
    {
        ...
        if ($params->get('remove_field_title_when_empty')){
            ...
            $html = ($file->created != '0000-00-00 00:00:00') ? str_replace('{created_date_title}', JText::_('COM_JDOWNLOADS_FE_DETAILS_CREATED_DATE_TITLE'), $html) : str_replace('{created_date_title}', '', $html);
            $html = ($file->downloads != '') ? str_replace('{hits_title}', JText::_('COM_JDOWNLOADS_FE_DETAILS_HITS_TITLE'), $html) : str_replace('{hits_title}', '', $html);
            $html = ($file->created_by) ? str_replace('{created_by_title}', JText::_('COM_JDOWNLOADS_FE_DETAILS_CREATED_BY_TITLE'), $html) : str_replace('{created_by_title}', '', $html);
            $html = ($file->modified_by) ? str_replace('{modified_by_title}', JText::_('COM_JDOWNLOADS_FE_DETAILS_MODIFIED_BY_TITLE'), $html) : str_replace('{modified_by_title}', '', $html);
    ...
    */
    ...
    {    
        ...
        if (($params->get('rating_only_for_regged') && $aid > 1) || !$params->get('rating_only_for_regged')) {
            $script='
    ...
    */
    public static function placeThumbs($body, $images, $type = 'list')
    {
        ...
        if ($images){ 
            ...
            foreach ($images as $image){
                $thumbnail =  JUri::base().'images/jdownloads/screenshots/thumbnails/'.$image.'" alt="'.substr($image,0,-4).$x; 
            ...
            $thumbnail = JUri::base().'images/jdownloads/screenshots/thumbnails/no_pic.gif" alt="no_pic';
    ...
    public static function getEditIcon($item)
    {
        ...
        if (property_exists($item, 'checked_out') && property_exists($item, 'checked_out_time') && $item->checked_out > 0 && $item->checked_out != $user->get('id')) {
            ...
            $button = JHtml::_('image', 'system/checked_out.png', 'checked_out.png', NULL, true);
            $date = JHtml::_('date', $item->checked_out_time);
            $tooltip = '<strong>'.JText::_('COM_JDOWNLOADS_CHECKED_OUT').'</strong><br />'.JText::sprintf('COM_JDOWNLOADS_CHECKED_OUT_BY', $checkoutUser->name).' <br /> '.$date;
            return '<span class="hasTooltip icon-checked_out.png" title="'.htmlspecialchars($tooltip, ENT_COMPAT, 'UTF-8').'">'.$button.'</span>';
        }

        $url    = 'index.php?option=com_jdownloads&task=download.edit&a_id='.$item->id.'&return='.base64_encode(urlencode($uri));
        ...
        $overlib .= $date;
        $overlib .= '&lt;br /&gt;';
        $overlib .= JText::sprintf('COM_JDOWNLOADS_BACKEND_FILESEDIT_CREATED_BY').' '.htmlspecialchars($author, ENT_COMPAT, 'UTF-8');

        if ($user_access_name){
            // Add Admin info for single user
            $overlib .= '&lt;br /&gt;';
            $overlib .= JText::sprintf('COM_JDOWNLOADS_EDIT_DOWNLOAD_SINGLE_USER_NAME').' '.htmlspecialchars($user_access_name, ENT_COMPAT, 'UTF-8');
        }

        $button = JHtml::_('link', JRoute::_($url), $text);

        $output = '<span class="hasTooltip icon-'.$icon.'" title="<strong>'.JText::_('COM_JDOWNLOADS_EDIT_DOWNLOAD').'</strong> &lt;br /&gt;'.$overlib.'">'.$button.'</span>';
    ...
    public static function getShorterFilename($filename)
    {
        ...
        if ($params->get('shortened_filename_length') > 14 && $filename){
            if (StringHelper::strlen($filename) > $params->get('shortened_filename_length')){
                ...
                $filename = JHtml::_('tooltip', $filename, JText::_('COM_JDOWNLOADS_FE_DETAILS_FILE_NAME_TITLE'), '', $short);
            }
        }
        return $filename;    
    } 
    ...
    */
    public static function useAntiLeeching()
    {
       ...
       if ($params->get('use_blocking_list') && $params->get('blocking_list') != '') {
           ...
           if (stristr($params->get('blocking_list'), $user_ip)){
               $blocking = true;
           }    
       }
       return $blocking;    
    ...
    public static function sendMailDownload($files)
    {
        ...
        for ($i=0; $i<count($files); $i++) {
           if ($files[$i]->license > 0){
               $mail_files .= "<div><li>".$files[$i]->title.' '.$files[$i]->release.'&nbsp;&nbsp;&nbsp;'.JText::_('COM_JDOWNLOADS_FE_DETAILS_LICENSE_TITLE').': '.$files[$i]->license_title.'&nbsp;&nbsp;&nbsp;'.JText::_('COM_JDOWNLOADS_FE_DETAILS_FILESIZE_TITLE').': '.$files[$i]->size.'</li></div>';
           } else {
               $mail_files .= "<div><li>".$files[$i]->title.' '.$files[$i]->release.'&nbsp;&nbsp;&nbsp;'.JText::_('COM_JDOWNLOADS_FE_DETAILS_FILESIZE_TITLE').': '.$files[$i]->size.'</li></div>';
           }  
        }
...
/**
    ...
    public static function updateLog($type, $files, $upload_data)
    {
        ...
        if ($type == 1){
            foreach ($files as $file){
                ...
                if ($file->extern_file != ''){
                    $db->setQuery("INSERT INTO #__jdownloads_logs (type, log_file_id, log_file_size, log_file_name, log_title, log_ip, log_datetime, log_user) VALUES ( '".$type."', '".$file->id."', '".$filesize."', '".$file->extern_file."', '".$db->escape($file->title)."',  '".$ip."', '".$date."', '".$user->get('id')."')");
                } else {
                    $db->setQuery("INSERT INTO #__jdownloads_logs (type, log_file_id, log_file_size, log_file_name, log_title, log_ip, log_datetime, log_user) VALUES ( '".$type."', '".$file->id."', '".$filesize."', '".$db->escape($filename_text)."', '".$db->escape($file->title)."',  '".$ip."', '".$date."', '".$user->get('id')."')");
                }    
                $db->execute();
            }
        } else {
            if ($type = 2){
                ...
                $db->setQuery("INSERT INTO #__jdownloads_logs (type, log_file_id, log_file_size, log_file_name, log_title, log_ip, log_datetime, log_user) VALUES ( '".$type."', '".$upload_data->id."', '".$filesize."', '".$db->escape($upload_data->url_download)."', '".$db->escape($upload_data->title)."',  '".$ip."', '".$date."', '".$user->get('id')."')");
                $db->execute();                
            }
    ...
    * @return text $layout   
    */
    public static function insertPagination($show_navigation, $show_pagination = 0, $show_pagination_results = 0, $pagination, $layout)
    {
        ...
        if (strpos($layout, '{page_navigation}') !== false){
            ...
            if ($show_navigation && $pagination->get('pages.total') > 1 && $show_pagination != '0' 
                || (!$show_navigation && $pagination->get('pages.total') > 1 && $show_pagination == '1') )
            {
                $page_navi_links = $pagination->getPagesLinks(); 
                if ($page_navi_links){
                    $page_navi_pages   = JText::_('COM_JDOWNLOADS_FE_FILELIST_TITLE_OVER_FILES_LIST').' '.$pagination->getPagesCounter();
    ...
    */
    public static function getFileTypeIconPath($selected_icon_set)
    {
        ...
        {
            case 2:
                $file_pic_folder = JURI::root().'images/jdownloads/fileimages/flat_1/';
                break;
            case 3:
                $file_pic_folder = JURI::root().'images/jdownloads/fileimages/flat_2/';
                break;
            default:
                $file_pic_folder = JURI::root().'images/jdownloads/fileimages/';
                break;
        }
    ...
    */ 
    ...
    {
        $plugin = JPluginHelper::getPlugin('content', 'jdownloadscart');
