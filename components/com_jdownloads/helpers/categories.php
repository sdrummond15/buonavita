...
class JDCategories
{
	...
	protected function _load($id)
	{
        ...
        // Define null and now dates to count correct amount numitems
        ...
        if (count($cats))
		{
			...
			foreach ($cats as $cat)
			{
                ...
                if ($cat->id > 1){
                    if ($cat->id > (int)$id || $cat->parent_id == (int)$id){
                        ...
                        // compute not already stored items again
                            ...
                            if ($this->_options['published'] == 1){
                                $query->where('cat.lft BETWEEN '.$cat->lft.' AND '.$cat->rgt.' AND cat.published = 1 AND cat.access IN (' . implode(',', $user->getAuthorisedViewLevels()) . ')');
                            } else {
                                $query->where('cat.lft BETWEEN '.$cat->lft.' AND '.$cat->rgt.' AND cat.access IN (' . implode(',', $user->getAuthorisedViewLevels()) . ')');
                            }    
                            ...
                            if ($children_ids)
                            {
                                $cat->subcatitems = (count($children_ids)-1);
                                ...
                                if ($this->_options['published'] == 1){
                                    ...
                                    if ($user->id > 0){
                                        ...
                                        if ($is_admin){
                                            ...
                                            $query->where('((files.access IN ('.$groups.') AND files.user_access = 0) OR (files.access != 0 AND files.user_access != 0))');
                                            //$query->where('c.access IN ('.$groups.')');
                                        } else {
                                            $query->where('((files.access IN ('.$groups.') AND files.user_access = 0) OR (files.access != 0 AND files.user_access = '.$db->quote($user->id). '))');
                                            //$query->where('c.access IN ('.$groups.')');
                                        }
                                    } else {    
                                        $query->where('files.access IN ('.$groups.')');
                                        //$query->where('c.access IN ('.$groups.')');
                                    }
                                    
                                    $query->where('files.catid IN ('.$ids.') AND files.published = 1');
                                    $query->where('(files.publish_up = ' . $nullDate . ' OR files.publish_up <= ' . $nowDate . ')')
                                          ->where('(files.publish_down = ' . $nullDate . ' OR files.publish_down >= ' . $nowDate . ')');
                                    ...
                                    $query->where('files.catid IN ('.$ids.') AND files.access IN (' . implode(',', $user->getAuthorisedViewLevels()) . ')');
                                    $query->where('(files.publish_up = ' . $nullDate . ' OR files.publish_up <= ' . $nowDate . ')')
                                           ->where('(files.publish_down = ' . $nullDate . ' OR files.publish_down >= ' . $nowDate . ')');
                                }
                                $db->setQuery($query);
...
class JDCategoryNode extends JObject
{
	...
	public function getAuthor($modified_user = false)
	{
		if ($modified_user)
		{
			return JFactory::getUser($this->modified_by);
		}
