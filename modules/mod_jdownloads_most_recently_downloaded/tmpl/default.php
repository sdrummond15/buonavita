...
* @copyright (C) 2008/2018 Arno Betz
    ...
    if ($files) {
        ...
        for ($i=0; $i<$sum_view; $i++) {
            ...
            if ($view_tooltip && $files[$i]->description){
                ...
                $link_text = '<a href="'.$link.'">'.JHTML::tooltip(strip_tags($files[$i]->description),JText::_('MOD_JDOWNLOADS_MOST_RECENTLY_DOWNLOADED_DESCRIPTION_TITLE'),$files[$i]->title.' '.$version.$files[$i]->release,$files[$i]->title.' '.$version.$files[$i]->release).'</a>';                
            } else {    
