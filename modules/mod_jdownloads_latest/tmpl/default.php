...
defined('_JEXEC') or die;
    ...
    if ($files) {
        ...
        for ($i=0; $i<count($files); $i++) {
            ...
            if ($view_date) {
				if ($files[$i]->created){
					if ($view_date_same_line){
						$html .= '<td style="text-align:'.$date_alignment.'"><small>'.JHTML::date($files[$i]->created, $date_format).'</small></td>'; 
					} else {
						$html .= '<tr style="vertical-align:top;"><td style="text-align:"'.$date_alignment.'"><small>'.JHTML::date($files[$i]->created, $date_format).'</small></td>';
					}
				}    
			} 
			$html .= '</tr>';
            ...
            if ($view_thumbnails){
                if ($first_image){
                    $thumbnail = '<a href="'.$link.'"><img class="img" src="'.$thumbfolder.$first_image.'" style="text-align:top;padding:5px;border:'.$border.';" width="'.$view_thumbnails_size.'" height="'.$view_thumbnails_size.'" alt="'.substr($files[$i]->title, 0, -4).'-'.$i.'" /></a>';
                } else {
                    // use placeholder
