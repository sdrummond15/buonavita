...
// this is an alternate layout without tables - it only uses <div> tags
    ...
    if ($files){
        ...
        for ($i=0; $i<count($files); $i++) {
			...
			if ($view_pics){
				...
				$files_pic = '<a href="'.$link.'"><img src="'.$file_pic_folder.$files[$i]->file_pic.'" width="'.$size.'" height="'.$size.'" style="border: 0px; vertical-align: top;"'.' alt="'.substr($files[$i]->file_pic, 0, -4).'-'.$i.'"/></a>';
            ...
            if ($view_tooltip && $files[$i]->description != ''){
				$link_text = '<a href="'.$link.'">'.JHTML::tooltip(strip_tags(substr($files[$i]->description,0,$view_tooltip_length)).$short_char,JText::_('MOD_JDOWNLOADS_LATEST_DESCRIPTION_TITLE'),$files[$i]->title.' '.$version.$files[$i]->release,$files[$i]->title.' '.$version.$files[$i]->release).'</a>';                
			} else {    
            ...
            if ($view_date) {
                if ($files[$i]->created){
                    if ($view_date_same_line){
                        $html .= '<div style="padding-bottom: 3px; float:'.$date_alignment.';"><small>'.JHTML::date($files[$i]->created, $date_format).'</small></div>'; 
                    } else {
                        $html .= '<div style="padding-bottom: 3px; text-align:'.$date_alignment.';"><small>'.JHTML::date($files[$i]->created, $date_format).'</small></div>';
                    }
                }    
            } 
            ...
            if ($view_thumbnails){
                if ($first_image){
                    $thumbnail = '<a href="'.$link.'"><img class="img" src="'.$thumbfolder.$first_image.'" style="padding:5px;border:'.$border.'" width="'.$view_thumbnails_size.'" height="'.$view_thumbnails_size.'" alt="'.substr($files[$i]->title, 0, -4).'-'.$i.'"/></a>';
                } else {
                    if ($view_thumbnails_dummy){
                        $thumbnail = '<a href="'.$link.'"><img class="img" src="'.$thumbfolder.'no_pic.gif" style="padding:5px;border:'.$border.'" width="'.$view_thumbnails_size.'" height="'.$view_thumbnails_size.'" alt="no_pic-'.$i.'"/></a>';
                    }    
                }
			...
			if ($cat_show_text2) {
                if ($cat_show_as_link){
                    if ($files[$i]->menuc_cat_itemid){
                        $html .= '<div style="padding-bottom: 3px; text-align:'.$alignment.'; font-size:'.$cat_show_text_size.'; color:'.$cat_show_text_color.';"><a href="index.php?option='.$option.'&amp;view=category&catid='.$files[$i]->catid.'&amp;Itemid='.$files[$i]->menuc_cat_itemid.'">'.$cat_show_text2.'</a></div>';
                    } else {
                        $html .= '<div style="padding-bottom: 3px; text-align:'.$alignment.'; font-size:'.$cat_show_text_size.'; color:'.$cat_show_text_color.';"><a href="index.php?option='.$option.'&amp;view=category&catid='.$files[$i]->catid.'&amp;Itemid='.$Itemid.'">'.$cat_show_text2.'</a></div>';
                    }
                } else {    
