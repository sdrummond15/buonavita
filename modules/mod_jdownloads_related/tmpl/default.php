    ...
    if (count($files) > 1 ){
        ...
        for ($i=0; $i<count($files); $i++) {
            ...
            if ($view_tooltip && $files[$i]->description != ''){
				$link_text = '<a href="'.$link.'">'.JHtml::tooltip(strip_tags(substr($files[$i]->description,0,$view_tooltip_length)).$short_char,JText::_('MOD_JDOWNLOADS_RELATED_DESCRIPTION_TITLE'),$files[$i]->title.' '.$version.$files[$i]->release,$files[$i]->title.' '.$version.$files[$i]->release).'</a>';                
			} else {    
            ...
            if ($view_date) {
                if ($files[$i]->created){
                    if ($view_date_same_line){
                        $html .= '<td style="text-align:'.$date_alignment.';"><small>&nbsp;'.substr(JHTML::Date($files[$i]->created,$date_format),0,10).'&nbsp;</small></td>';
                    } else {
                        $html .= '</tr><tr><td style="text-align:'.$date_alignment.';"><small>'.substr(JHTML::Date($files[$i]->created,$date_format),0,10).'</small></td>';
                    } 
                }    
            } 
            $html .= '</tr>';
