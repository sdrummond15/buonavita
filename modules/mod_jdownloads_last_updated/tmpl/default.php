    ...
    if ($files) {
        ...
        for ($i=0; $i<count($files); $i++) {
			...
			if ($view_tooltip && $files[$i]->description != ''){
                $link_text = '<a href="'.$link.'">'.JHTML::tooltip(strip_tags(substr($files[$i]->description,0,$view_tooltip_length)).$short_char,JText::_('MOD_JDOWNLOADS_LAST_UPDATED_DESCRIPTION_TITLE'),$files[$i]->title.' '.$version.$files[$i]->release,$files[$i]->title.' '.$version.$files[$i]->release).'</a>';                
            } else {    
			...
			if ($view_date) {
                if ($files[$i]->modified){
                    if ($view_date_same_line){
                        $html .= '<td style="text-align:'.$date_alignment.';"><small>'.$view_date_text.'&nbsp;'.substr(JHTML::Date($files[$i]->modified,$date_format),0,10).'</small></td>';
                    } else {
                        $html .= '</tr><tr><td style="text-align:'.$date_alignment.';"><small>'.$view_date_text.'&nbsp;'.substr(JHTML::Date($files[$i]->modified,$date_format),0,10).'</small></td>';
                    }    
                }
            }
            $html .= '</tr>';
