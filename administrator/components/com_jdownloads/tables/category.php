...
/**
    ...
    public function checkData($isNew, $auto_added = false)
    {
        ...
        $table = JTable::getInstance('category', 'jdownloadsTable', array('dbo' => $this->getDbo()));
        if ($table->load(array('alias' => $this->alias, 'parent_id' => (int) $this->parent_id)) && ($table->id != $this->id || $this->id == 0)){
            if ($auto_added){
                ...
                $this->setError(JText::_('COM_JDOWNLOADS_BACKEND_ERROR_CATEGORY_UNIQUE_ALIAS'));
                return false;
            }
        }        
	        ...
	        if (isset($file['tmp_name']['picnew'])){
		        ...
		        if ($file['tmp_name']['picnew'] != ''){
		            ...
		            if (JDownloadsHelper::fileIsPicture($pic['name'])){
		                ...
		                $pic['name'] = JFile::makeSafe($pic['name']);
		
		                // Replace all spaces with underscores to prevent problems
		                $pic['name'] = str_replace(' ', '_', $pic['name']);
		                
		                if (!JFile::upload($pic['tmp_name'], $upload_dir.$pic['name'], false, true)){
