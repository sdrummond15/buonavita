...
class JFormFieldModal_Category extends JFormField
{
	...
	protected function getInput()
	{
		...
		$linkCategories = 'index.php?option=com_jdownloads&amp;view=categories&amp;layout=modal&amp;tmpl=component&amp;' . JSession::getFormToken() . '=1';
		$linkCategory   = 'index.php?option=com_jdownloads&amp;view=category&amp;layout=modal&amp;tmpl=component&amp;' . JSession::getFormToken() . '=1';
		$modalTitle     = JText::_('COM_JDOWNLOADS_CHANGE_CATEGORY');
		...
		$title = empty($title) ? JText::_('COM_JDOWNLOADS_SELECT_A_CATEGORY') : htmlspecialchars($title, ENT_QUOTES, 'UTF-8');
		...
		if ($allowSelect)
		{
			$html .= '<button'
				...
				. ' title="' . JHtml::tooltipText('COM_JDOWNLOADS_CHANGE_CATEGORY') . '">'
		...
		if ($allowNew)
		{
			$html .= '<button'
				...
				. ' title="' . JHtml::tooltipText('COM_JDOWNLOADS_NEW_CATEGORY') . '">'
		...
		if ($allowEdit)
		{
			$html .= '<button'
				...
				. ' title="' . JHtml::tooltipText('COM_JDOWNLOADS_EDIT_CATEGORY') . '">'
		...
		if ($allowNew)
		{
			$html .= JHtml::_(
				...
				'ModalNew' . $modalId,
				array(
					'title'       => JText::_('COM_JDOWNLOADS_NEW_CATEGORY'),
					'backdrop'    => 'static',
					'keyboard'    => false,
					'closeButton' => false,
					'url'         => $urlNew,
		...
		if ($allowEdit)
		{
			$html .= JHtml::_(
				...
				'ModalEdit' . $modalId,
				array(
					'title'       => JText::_('COM_JDOWNLOADS_EDIT_CATEGORY'),
					'backdrop'    => 'static',
					'keyboard'    => false,
					'closeButton' => false,
					'url'         => $urlEdit,
		...
		$html .= '<input type="hidden" id="' . $this->id . '_id"' . $class . ' data-required="' . (int) $this->required . '" name="' . $this->name
			. '" data-text="' . htmlspecialchars(JText::_('COM_JDOWNLOADS_SELECT_A_CATEGORY', true), ENT_COMPAT, 'UTF-8') . '" value="' . $value . '" />';
