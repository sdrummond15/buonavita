...
class JFormFieldItemLanguage extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		$helper = JDAssociationsHelper::getExtensionHelper($extensionName);
		...
		$associations = JDAssociationsHelper::getAssociationList($extensionName, $typeName, $referenceId);

		// Check if user can create items in this component item type.
		$canCreate = JDAssociationsHelper::allowAdd($extensionName, $typeName);
		...
		foreach ($existingLanguages as $langCode => $language)
		{
			...
			if (isset($associations[$language->lang_code]))
			{
				...
				$canEdit = JDAssociationsHelper::allowEdit($extensionName, $typeName, $itemId);

				// Check if item can be checked out
				$canCheckout = JDAssociationsHelper::canCheckinItem($extensionName, $typeName, $itemId);
