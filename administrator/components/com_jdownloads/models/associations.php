 ...
 * @since  3.7.0
 */
class JDownloadsModelAssociations extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function purge($context = '', $key = '')
	{
		...
		{
			$app->enqueueMessage(JText::_('COM_JDOWNLOADS_ASSOCIATIONS_PURGE_FAILED'), 'error');

			return false;
		}

		$app->enqueueMessage(
			JText::_((int) $db->getAffectedRows() > 0 ? 'COM_JDOWNLOADS_ASSOCIATIONS_PURGE_SUCCESS' : 'COM_JDOWNLOADS_ASSOCIATIONS_PURGE_NONE'),
			'message'
		);
	...
	public function clean($context = '', $key = '')
	{
		...
		foreach ($assocKeys as $value)
		{
			...
			{
				$app->enqueueMessage(JText::_('COM_JDOWNLOADS_ASSOCIATIONS_DELETE_ORPHANS_FAILED'), 'error');
		...
		$app->enqueueMessage(
			JText::_($count > 0 ? 'COM_JDOWNLOADS_ASSOCIATIONS_DELETE_ORPHANS_SUCCESS' : 'COM_JDOWNLOADS_ASSOCIATIONS_DELETE_ORPHANS_NONE'),
			'message'
		);
