 ...
 * @since  3.7.0
 */
class JDAssociationsHelper extends JHelperContent
{
	/**
	...
	public static function getAssociationHtmlList($extensionName, $typeName, $itemId, $itemLanguage, $addLink = true, $assocLanguages = true)
	{
		...
		foreach ($languages as $langCode => $language)
		{
			...
			if (isset($items[$langCode]))
			{
				...
				$additional .= $addLink && $allow ? JText::_('COM_JDOWNLOADS_ASSOCIATIONS_EDIT_ASSOCIATION') : '';
			}
			else
			{
				$items[$langCode] = array();

				$title      = JText::_('COM_JDOWNLOADS_ASSOCIATIONS_NO_ASSOCIATION');
				$additional = $addLink ? JText::_('COM_JDOWNLOADS_ASSOCIATIONS_ADD_NEW_ASSOCIATION') : '';
	...
	public static function getSupportedExtension($extensionName)
	{
		...
		foreach ($types as $typeName)
		{
			...
			{
				$title = JText::_('COM_JDOWNLOADS_CATEGORIES');
				$context     = 'category';
			} else {
                $title = JText::_('COM_JDOWNLOADS_DOWNLOADS');
                //$context     = 'category';
			}
